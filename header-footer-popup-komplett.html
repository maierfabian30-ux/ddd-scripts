...
#ddd-nav .nav-mobile .nav-mobile-cta {
  ...
  color: #0f2318; font-size: 15px; font-weight: 700;
...
#ddd-footer .f-bottom {
  position: relative; z-index: 1; padding: 24px 7%;
...
#ddd-popup .pp-header::before {
  content: ''; position: absolute; inset: 0; pointer-events: none;
  background-image: linear-gradient(rgba(255,255,255,0.03) 1px,transparent 1px),
                    linear-gradient(90deg,rgba(255,255,255,0.03) 1px,transparent 1px);
  background-size: 50px 50px;
}
#ddd-popup .pp-header::after {
...
#ddd-popup .pp-orb {
  ...
  background: radial-gradient(circle,rgba(82,168,110,0.15) 0%,transparent 65%);
  pointer-events: none;
}
...
#ddd-popup .pp-title {
  font-size: 28px; font-weight: 700; line-height: 1.15; color: #fff;
...
#ddd-popup .pp-perk-icon {
  width: 28px; height: 28px; border-radius: 8px; flex-shrink: 0;
  background: rgba(45,90,61,0.09); border: 1px solid rgba(45,90,61,0.15);
...
#ddd-popup .pp-input {
  ...
  border: 1.5px solid rgba(15,35,24,0.12); border-radius: 12px;
  font-size: 14px; font-family: inherit; color: #0f2318;
...
#ddd-popup .pp-submit {
  ...
  box-shadow: 0 6px 20px rgba(15,35,24,0.2);
  transition: transform 0.25s, box-shadow 0.25s, filter 0.25s;
  display: flex; align-items: center; justify-content: center; gap: 10px;
}
...
#ddd-popup .pp-success-icon {
  width: 56px; height: 56px; border-radius: 50%; margin: 0 auto 16px;
  background: rgba(45,90,61,0.1); border: 2px solid rgba(45,90,61,0.25);
  display: flex; align-items: center; justify-content: center;
}
...
#ddd-popup .pp-skip {
  ...
  font-size: 12px; color: rgba(15,35,24,0.35);
  cursor: pointer; background: none; border: none;
  font-family: inherit; width: 100%; transition: color 0.2s;
}
...
(function() {
  ...
  ════════════════════════════════ */
  var POPUP_DELAY    = 30;           /* Sekunden bis Pop-up erscheint */
  var COOLDOWN_DAYS  = 7;            /* Tage bis Pop-up nach Schließen wieder erscheint */
  var SUBSCRIBED_KEY = 'ddd_nl_sub'; /* localStorage Key nach Anmeldung */

  /* ── Brevo URL hier eintragen sobald bereit ──
     Sieht so aus: https://sibforms.com/serve/XXXXXXX  */
    ...
    '<div id="ddd-footer">' +
      ...
      '<div class="f-main">' +
        '<div class="f-brand">' +
          ...
          '<div class="f-links">' +
            '<a href="tel:062011884760" class="f-link">' +
              '<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M13.5 10.5v2a1.33 1.33 0 01-1.45 1.33 13.2 13.2 0 01-5.75-2.05 13 13 0 01-4-4 13.2 13.2 0 01-2.05-5.78A1.33 1.33 0 011.55 1H3.5a1.33 1.33 0 011.33 1.15c.09.7.25 1.38.49 2.03a1.33 1.33 0 01-.3 1.4L4.09 6.5a10.67 10.67 0 004 4l.92-.92a1.33 1.33 0 011.4-.3c.65.23 1.33.4 2.03.49A1.33 1.33 0 0113.5 11v-.5z"/></svg>' +
              '06201 – 188476' +
            '</a>' +
            '<a href="mailto:info@ddd-consulting.de" class="f-link">' +
              '<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M2.67 2.67h10.66c.74 0 1.34.6 1.34 1.33v8c0 .74-.6 1.33-1.34 1.33H2.67c-.74 0-1.34-.6-1.34-1.33V4c0-.74.6-1.33 1.34-1.33z"/><polyline points="14.67,4 8,8.67 1.33,4"/></svg>' +
              'info@ddd-consulting.de' +
            '</a>' +
            '<a href="https://www.google.com/maps/dir/?api=1&destination=Weimarer+Str.+16,+69469+Weinheim" target="_blank" class="f-link">' +
              '<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M8 1.33A4.67 4.67 0 0112.67 6c0 3.5-4.67 8.67-4.67 8.67S3.33 9.5 3.33 6A4.67 4.67 0 018 1.33z"/><circle cx="8" cy="6" r="1.67"/></svg>' +
              'Weimarer Str. 16, 69469 Weinheim' +
            '</a>' +
          '</div>' +
        '</div>' +
        '<div class="f-col">' +
          '<span class="f-col-title">Navigation</span>' +
          '<ul>' +
            '<li><a href="/">Startseite</a></li>' +
            '<li><a href="/dienstleistungen/">Dienstleistungen</a></li>' +
            '<li><a href="/investment-ideen/">Investment-Ideen</a></li>' +
        ...
        '</div>' +
        '<div class="f-col">' +
          '<span class="f-col-title">Leistungen</span>' +
          '<ul>' +
        ...
        '</div>' +
        '<div class="f-col">' +
          '<span class="f-col-title">Öffnungszeiten</span>' +
          '<div class="f-hours">' +
      ...
      '<div class="f-bottom">' +
        '<p class="f-copy">&copy; ' + new Date().getFullYear() + ' <span>DDD-Consulting</span> &middot; Dr. Dirk Dauenheimer &middot; Alle Rechte vorbehalten</p>' +
  ...
  var POPUP_HTML =
    '<div id="ddd-popup-overlay">' +
      '<div id="ddd-popup">' +
        '<button class="pp-close" aria-label="Schließen">' +
        ...
        '<div class="pp-body">' +
          '<div class="pp-perks">' +
            ...
            '<div class="pp-perk"><div class="pp-perk-icon"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="6.5"/><path d="M5 8L7 10L11 6"/></svg></div>Kein Spam – max. 2&times; pro Monat, jederzeit abmeldbar</div>' +
          '</div>' +
